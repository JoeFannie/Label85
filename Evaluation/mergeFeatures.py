...
output_lmdb_path = '/home/caojiajiong/attribute_learning/AUX_test_lmdb'
label = lmdb.open('/home/caojiajiong/attribute_learning/test_label_new_lmdb')
label_txn = label.begin()
label_cursor = label_txn.cursor()
datum = caffe.proto.caffe_pb2.Datum()
data_label = np.zeros((17311, 80,1,1))
count = 0

for key, value in label_cursor:
  datum.ParseFromString(value)
  data_label[count, :] = caffe.io.datum_to_array(datum).reshape(80,1,1)
  count = count + 1
...
loss_count = 0;
data_loss = np.zeros((17311, 80,1,1))
for loss in loss_list:
	loss_name = '/home/caojiajiong/attribute_learning/features/MCNN_30000/' + loss
...
np.savetxt('/home/caojiajiong/attribute_learning/features/MCNN_30000/loss_all.txt', data_loss, delimiter = ' ')
...
# for idx in range(int(math.ceil(N/1000.0))):
  ...
  # with in_db_label.begin(write=True) as in_txn:
      # for label_idx, label_ in enumerate(data_merge[(1000*idx):(1000*(idx+1))]):
			# im_dat = caffe.io.array_to_datum(np.array(label_).astype(float).reshape(160,1,1)
