...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	...
	if (this->phase_ == TRAIN) {
        	iter_ += 1;
		iter_ = (round(1.0 / decay_)) < iter_ ? (round(1.0 / decay_)) : iter_;
		scale_factor = Dtype(1.0) / iter_;
		caffe_cpu_axpby(batch_statistic_.count(), scale_factor, batch_statistic_.cpu_data(), Dtype(1) - scale_factor, this->blobs_[2]->mutable_cpu_data());
	...
	if (this->phase_ == TRAIN) {
                caffe_cpu_axpby(batch_statistic_.count(), scale_factor, batch_statistic_.cpu_data(), Dtype(1) - scale_factor,
	...
	if (this->phase_ == TEST && moving_average_) {
		...
		Dtype alpha = m > 1 ? Dtype(m) / (m - 1) : 1;
		caffe_scal(batch_statistic_.count(), alpha, batch_statistic_.mutable_cpu_data());
	}
	// std
