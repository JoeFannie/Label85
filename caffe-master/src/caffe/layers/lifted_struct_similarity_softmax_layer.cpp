...
void LiftedStructSimilaritySoftmaxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i=0; i<N_; i++){
    for (int j=i+1; j<N_; j++){
      ...
      if (label_mat[i][j]){
        Dtype dist_pos = sqrt(std::max(Dtype(0), dot_.cpu_data()[i*N_ + j]));
        ...
        for (int k=0; k<N_; k++){
          if (!label_mat[i][k]){
            loss_aug_inference_.mutable_cpu_data()[neg_idx] = margin - sqrt(std::max(Dtype(0), dot_.cpu_data()[i*N_ + k]));
            neg_idx++;
          }
        }
        ...
        for (int k=0; k<N_; k++){
          if (!label_mat[j][k]){
            loss_aug_inference_.mutable_cpu_data()[neg_idx] = margin - sqrt(std::max(Dtype(0), dot_.cpu_data()[j*N_ + k]));
            neg_idx++;
          }
        }
	...
	if (this_loss <= 0) continue;
        ...
        scaler = (dist_pos > 0)? Dtype(2.0)*this_loss / dist_pos : 0;
        // update x_i
	    ...
	    assert(sum_exp != 0);
            ...
            scaler = dot_.cpu_data()[i*N_ + k] > 0 ? dJ_dDik / sqrt(dot_.cpu_data()[i*N_ + k]) : 0;
	    ...
	    assert(sum_exp != 0);
            ...
            scaler = dot_.cpu_data()[j*N_ + k] > 0 ? dJ_dDjk / sqrt(dot_.cpu_data()[j*N_ + k]) : 0;
