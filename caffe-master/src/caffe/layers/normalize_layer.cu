...
void NormalizeLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  //}
	...
	for (int i = 0; i<n; ++i) {
		...
		Dtype L2norm = Dtype(pow(normsqr, -0.5));
		caffe_gpu_mul(d, top_data + i*d, top_data + i*d, bottom_diff + i*d);
		caffe_gpu_mul(d, bottom_diff + i*d, top_diff + i*d, bottom_diff + i*d);
		caffe_gpu_sub(d, top_diff + i*d, bottom_diff + i*d, bottom_diff + i*d);
		caffe_gpu_scale(d, L2norm, bottom_diff + i*d, bottom_diff + i*d);
	}
