...
namespace caffe {
	...
	template <typename Dtype>
	__global__ void ELUBackward(const int n, const Dtype* in_diff, const Dtype* in_data,
		const Dtype* out_data, Dtype* out_diff, Dtype alpha) {
		CUDA_KERNEL_LOOP(index, n) {
			out_diff[index] = in_diff[index] * ((out_data[index]>0) ? 1 : (in_data[index] + alpha));
		}
	}
