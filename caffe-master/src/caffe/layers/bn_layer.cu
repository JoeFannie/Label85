...
namespace caffe {
	...
	void BNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (this->phase_ == TRAIN) {
	  		...
	  		caffe_gpu_axpby(batch_statistic_.count(), scale_factor, batch_statistic_.gpu_data(), Dtype(1) - scale_factor,
	...
	caffe_gpu_axpby(batch_statistic_.count(), scale_factor, batch_statistic_.gpu_data(), Dtype(1) - scale_factor,
      ...
      //                this->blobs_[3]->mutable_gpu_data());
		...
		if (this->phase_ == TEST && moving_average_) {
			...
			caffe_gpu_scal(batch_statistic_.count(), alpha, batch_statistic_.mutable_gpu_data());
		}
