...
void NormalizeLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  //}
	...
	for (int i = 0; i<n; ++i) {
		...
		caffe_mul(d, top_data + i*d, top_data + i*d, bottom_diff + i*d);
		caffe_mul(d,bottom_diff+i*d, top_diff+i*d, bottom_diff+i*d);
		caffe_sub(d, top_diff+i*d, bottom_diff+i*d, bottom_diff+i*d);

		caffe_cpu_scale(d, L2norm, bottom_diff + i*d, bottom_diff + i*d);
	}
}
